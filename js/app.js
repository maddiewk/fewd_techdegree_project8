...
//create modal window for each employee
 createModalWindow(profile) {
  ...
  const birthdayFormat = (bday) => {
    ...
    return `${emplBday[1]}/${emplBday[2]}/${emplBday[0].slice(0,4)}`; // 0000
  }
...
const getRandomEmployee = () => {
  return fetch('https://randomuser.me/api/?results=12&inc=name,location,email,picture,cell,dob&nat=NZ,US,CA,BR,IR')
